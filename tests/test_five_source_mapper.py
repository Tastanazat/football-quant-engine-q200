...
def test_five_source_mapper_accepts_all_sources():
    ...
    assert result.statshub_home is not None
    assert result.statshub_away is not None
...
def test_odds_are_not_inserted_into_canonical_model_values():
    ...
    assert (
        "home" not in result.canonical_values
    )

    assert (
        "draw" not in result.canonical_values
    )

    assert (
        "away" not in result.canonical_values
    )
