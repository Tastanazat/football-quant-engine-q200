...
def test_lambda_formula():
    s = TeamStats(2.0, 1.0, 1.2, 1.5, 1.8, 1.1, 1.0, 1.4)
...
def test_probabilities_sum_to_one():
    p = poisson_match_probabilities(1.5, 1.1)
...
def test_monte_carlo_minimum():
    with pytest.raises(ValueError):
        simulate_match(1.2, 1.0, iterations=99_999)


def test_no_vig_sums_to_one():
    p = implied_probabilities({"HOME": 2.0, "DRAW": 3.5, "AWAY": 4.0})
    assert sum(p.values()) == pytest.approx(1.0)


def test_quarter_kelly_respects_two_percent_cap():
    result = quarter_kelly(0.70, 2.0, 50_000)
    assert result["stake"] <= 1000


def test_minimum_odds_filter():
    with pytest.raises(ValueError):
        select({"HOME": .60, "DRAW": .20, "AWAY": .20},
               {"HOME": 1.40, "DRAW": 4.0, "AWAY": 5.0}, 50_000)


def test_ev_can_create_eligible_selection():
    rows = select({"HOME": .60, "DRAW": .20, "AWAY": .20},
                  {"HOME": 2.0, "DRAW": 4.0, "AWAY": 5.0},
                  50_000, uncertainty="LOW")
...
def test_pipeline_keeps_model_independent_of_odds():
    ...
    s = TeamStats(2, 1, 1.2, 1.4, 1.7, 1.0, 1.1, 1.3)
    p = Q200Pipeline(s)
    before = p.snapshot
    p.analyze_odds({"HOME": 2.0, "DRAW": 3.5, "AWAY": 4.0}, 50_000)
